...
public final class PlantCooldowns {
    ...
    private final Map<PlantTypeEnum, Double> remainingSecondsByType = new EnumMap<>(PlantTypeEnum.class);

    /** 查询指定植物当前剩余的冷却秒数。 */
    public double remaining(PlantTypeEnum type) {
        return remainingSecondsByType.getOrDefault(type, 0.0);
    }

    /** 种植成功后把指定植物的冷却时间重置为完整冷却秒数。 */
    public void start(PlantTypeEnum type, double cooldown) {
